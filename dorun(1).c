    ...
    while( 1 ) {
        ...
        switch( opcode ){              // EXECUTE
            ...
            case 0x04: /* ADD */
            	...
            	pc++;
            	if(Regs[reg3] == 0) { zero = '1'; }
                else { zero = '0'; }
            	if(Regs[reg3] < 0) { positivo = '0'; }
                else{ positivo = '1'; }
                break;
            ...
            case 0x09: /* JZ */
            	if(zero == '1'){ pc = address; }
                else { pc++; }
                break;
                
    	    case 0x0A: /* JNZ */
    	    	if(zero == '0'){ pc = address; }
                else { pc++; }
                break;
				
            case 0x0B:  /* JG */
                if((positivo == '1') && (zero == '0')){ pc = address; }
                else { pc++; }
                break;
            
            case 0x0C: /* JGE */
                if((positivo == '1')){ pc = address; }
                else { pc++; }
                break;
				
            case 0x0D: /* JB */
                if((positivo == '0') && (zero == '0')){ pc = address; }
                else { pc++; }
                break;

                // ZERO -> TRUE => zero == '0'
                // POSITIVE -> TRUE => positive !=  '0'

            case 0x0E: /* JBE */ // when negative! or when 0
                if((zero == '1' && positivo == '1') || (zero == '0' && positivo == '0')) { pc = address; }
                else { pc++; }
                break;
